...
function getDTEnd(options) {
  ...
  var end = start.setHours(start.getHours() + 2)
...
function createEvent(options, filepath, cb) {
  ...
  if (filepath) {
    dest = path.join(filepath, options[0].filename);
  } else if (options[0].filename) {
    dest = setFileExtension(path.join(TMPDIR, options[0].filename));
  } else {
